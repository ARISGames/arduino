...
void printPGM(Print* stream, PGM_P str) {
    for (uint8_t c; (c = pgm_read_byte(str)); str++) stream->print(c);
}
//------------------------------------------------------------------------------
...
void printLine(Print* stream) {
  print_P(stream, "//");
  for (uint8_t i = 0; i < 78; i++) stream->print('-');
  stream->println();
}
//------------------------------------------------------------------------------
void printI2C(Print* stream, uint8_t* i2c) {
...
//------------------------------------------------------------------------------
void printSPI(Print* stream, uint8_t* spi) {
